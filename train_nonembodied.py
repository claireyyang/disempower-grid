"""
Train non-embodied assistant environments.
"""
...
from environments.nonembodied_assistant.environment import reset_envs_specific_positions
from environments.nonembodied_assistant.environment import step_envs
from utils.file_utils import load_specific_positions_from_file
from environments.nonembodied_assistant.observation import (
    convert_state_to_grid_observation,
    convert_state_to_partial_grid_observation,
    convert_state_to_flattened_vector,
)
from environments.nonembodied_assistant.nonembodied_multiagent_gridworld import (
...
from environments.nonembodied_assistant.empowerment_estimator import (
    estimate_empowerment_monte_carlo,
    estimate_empowerment_variance_proxy,
)
from environments.nonembodied_assistant.choice_estimator import (
    estimate_discrete_choice,
    estimate_entropic_choice,
)
from environments.nonembodied_assistant.minimax_empowerment_estimator import (
    estimate_minimax_regret_empowerment,
)
import pickle
import orbax.checkpoint as ocp
from environments.nonembodied_assistant.viz import (
    create_frame_data,
...
def make_ppo_train_phase1(
    envs: NonEmbodiedMultiAgentGridWorld, helper_policy: RandomPolicy, wandb_id: str
):
...
def make_ppo_train_phase2(
    envs: NonEmbodiedMultiAgentGridWorld,
    ...
    def train_phase2(rng, specific_position_reset=None):
        ...
        def _update_step(runner_state, update_idx):
            ...
            # Calculate freeze action frequency per timestep
            freeze_action_value = (
                envs.HelperActions.FREEZE_AGENT_1.value if not envs.no_freeze else -1
            )
            helper_actions = traj_batch.action  # Shape: (NUM_STEPS, NUM_ENVS)

            # Calculate freeze frequency per timestep (average across environments)
            if not envs.no_freeze:
                freeze_mask = (
                    helper_actions == freeze_action_value
                )  # Shape: (NUM_STEPS, NUM_ENVS)
                freeze_frequency_per_timestep = jnp.mean(
                    freeze_mask, axis=1
                )  # Shape: (NUM_STEPS,) - average across envs
                freeze_frequency_mean = jnp.mean(
                    freeze_frequency_per_timestep
                )  # Scalar - average across timesteps
            else:
                freeze_frequency_per_timestep = jnp.zeros(config["NUM_STEPS"])
                freeze_frequency_mean = 0.0
            ...
            logging_metrics = {
                ...
                "freeze_frequency_per_timestep": freeze_frequency_per_timestep,  # Freeze frequency per timestep
                "freeze_frequency_mean": freeze_frequency_mean,  # Mean freeze frequency
        ...
        for update_idx in range(config["NUM_UPDATES"]):
            ...
            # Log freeze action frequency
            if "freeze_frequency_mean" in epoch_logging_metrics:
                epoch_metrics[f"phase2_{helper_name}/actions/freeze_frequency_mean"] = (
                    float(epoch_logging_metrics["freeze_frequency_mean"])
                )

            # Log freeze frequency per timestep for detailed analysis
            if "freeze_frequency_per_timestep" in epoch_logging_metrics:
                freeze_freq_per_step = epoch_logging_metrics[
                    "freeze_frequency_per_timestep"
                ]
                # Log first few timesteps as examples
                for step_idx in range(min(10, len(freeze_freq_per_step))):
                    epoch_metrics[
                        f"phase2_{helper_name}/actions/freeze_frequency_step_{step_idx}"
                    ] = float(freeze_freq_per_step[step_idx])
            ...
            csv_data = {
                ...
                "freeze_frequency_mean": float(
                    epoch_logging_metrics["freeze_frequency_mean"]
                ),
            }
...
if __name__ == "__main__":
    ...
    config = vars(args)
    config["env_name"] = "non_embodied_multiagent_gridworld"
